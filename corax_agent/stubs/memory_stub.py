...
The ``none`` memory backend: it stores nothing and always reports empty.
Useful so the memory registry is never empty and call sites can assume a
backend exists. A SQLite / vector store replaces it under the same role.
"""
...
class MemoryStub:
    ...
    async def health(self) -> StubHealth:
        return StubHealth(id=self.id, kind=self.kind, detail="no persistent memory")
