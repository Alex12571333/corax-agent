...
# Candidate config filenames, in the order we look for them. ``agent.*`` is
# kept as a fallback so configs from earlier scaffolds still load.
CONFIG_CANDIDATES = (
    "corax.yaml",
    "corax.yml",
    "corax.json",
...
def default_config_path(base: Path | None = None) -> Path:
    ...
    return base / ("corax.yaml" if yaml_lite.HAS_PYYAML else "corax.json")
