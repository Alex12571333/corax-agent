...
:mod:`corax.ui.menu` focused on flow control.
"""
...
MAIN_MENU = [
    "",
    "Corax Setup",
    "  Advanced settings. Guided onboarding: corax setup",
    "",
    "  1. Runtime Status",
    "  2. Runtime Paths & Logs",
    "  3. Planner",
    "  4. Memory",
    "  5. Connectors",
    "  6. Capabilities",
    ...
    "",
    "Commands: corax | corax setup | corax settings | corax gateway | corax status",
    "",
]
...
def llm_screen(config: AgentConfig) -> list[str]:
    llm = config.llm
    return [
        "LLM Local Connector (Spark)",
        "",
        f"  1. base_url     : {llm.base_url}",
        f"  2. model        : {llm.model}",
        f"  3. image input  : {_mark(llm.enable_image)}",
        f"  4. video input  : {_mark(llm.enable_video)}",
        "",
